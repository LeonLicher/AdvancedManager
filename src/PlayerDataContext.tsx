    ...
    const loadPlayers = async () => {
      try {
        ...
        const processedPlayers = Object.values(rawData.players)
          .filter((player: any): player is DetailedPlayer => 
            player && typeof player === 'object' && 'i' in player
          )
          .sort((a, b) => b.tp - a.tp);

        setPlayers(processedPlayers);
        sessionStorage.setItem('detailedPlayers', JSON.stringify(processedPlayers));
        setIsLoading(false);
      } catch (err) {
