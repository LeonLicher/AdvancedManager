    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [squadResponse, budgetResponse] = await Promise.all([
          httpClient.get<{ it: SquadPlayer[] }>(`/v4/leagues/${leagueId}/squad`),
          httpClient.get<BudgetResponse>(`/v4/leagues/${leagueId}/me/budget`)
        ]);
  ...
  const calculateUpdatesUntilFriday = () => {
    ...
    while (checkDate < target) {
      ...
      if (checkDate.getDate() === now.getDate()) {
        ...
        if (checkDate.getTime() + (updateTime * 60 * 1000) < target.getTime()) {
          updates++;
        }
      }
    ...
    <div className="calculator">
      <div className="balance-info">
        <div className="current-stats">
          <h3>Kontostand: <span>{budget?.b.toLocaleString()} €</span></h3>
          <h3>Marktwertänderung ausgewählte Spieler: <span>{calculateDailyMarketValueChange().toLocaleString()} €</span></h3>
        </div>
        
        <div className="projected-stats">
          <h3>Kontostand am Freitag: 
            <span>{calculateProjectedTotalBudget().toLocaleString()}  €</span>
          </h3>
        </div>
      </div>
      ...
      <div className="controls">
        <div className="selection-controls">
          ...
          <div className="selected-count">
            {selectedPlayers.length} von {players.length} Spielern ausgewählt
          </div>
        </div>
        ...
        <div className="sorting-controls">
          <div className="sort-group">
            ...
            <Button
              variant={sortBy === 'mv-desc' ? 'default' : 'outline'}
              size="sm"
            ...
            <Button
              variant={sortBy === 'mv-asc' ? 'default' : 'outline'}
              size="sm"
          ...
          <div className="sort-group">
            ...
            <Button
              variant={sortBy === 'daily-desc' ? 'default' : 'outline'}
              size="sm"
            ...
            <Button
              variant={sortBy === 'daily-asc' ? 'default' : 'outline'}
              size="sm"
            ...
            <div className="player-info">
              ...
              <div className="stats">
                ...
                <div className="stat">
                  ...
                  <span className={player.tfhmvt > 0 ? 'positive' : 'negative'}>
              ...
              <div className="matches">
                <span className="matches-label">Next Matches:</span>
                <div className="opponents-scroll">
                  {nextMatches[player.tn]?.slice(0, 6).map((match, idx) => (
                    <div 
                      key={idx} 
