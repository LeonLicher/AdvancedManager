...
const PlayerStats: React.FC<PlayerStatsProps> = ({
    ...
    const filteredAndFormattedData = useMemo(() => {
        ...
        return stats.marketValues
            .filter(
                (value) => !showLast3Weeks || new Date(value.d) >= threeWeeksAgo
            )
            .map((value) => ({
                ...value,
                d: new Date(value.d),
    ...
    return (
        <div className="player-stats">
            <h3>
                {player.fn} {player.n} - Market Value History
            </h3>
            <p>
                Highest Value: {formatPrice(stats.mvHigh)} on{' '}
                {format(new Date(stats.mvHighDate), 'dd.MM.yyyy')}
            </p>
            <p>
                Lowest Value: {formatPrice(stats.mvLow)} on{' '}
                {format(new Date(stats.mvLowDate), 'dd.MM.yyyy')}
            </p>
            <button
