...
const CustomTooltip = React.memo(({ active, payload, selectedPlayer, tooltipPos, httpClient }: TooltipProps) => {
  const shouldShow = (active && payload && payload.length) || (selectedPlayer && tooltipPos);
  const player = shouldShow ? (payload?.[0]?.payload || selectedPlayer) : null;
...
const AvailablePlayers: React.FC = () => {
  ...
  const scatterData = useMemo(() => 
    players
      .filter(player => selectedPosition === 0 || player.pos === selectedPosition)
      .map(player => ({
        ...player,
        ppm: player.mv > 0 ? 
          Number(((player.tp) / (player.mv / 1000000)).toFixed(2)) 
          : 0,
        fill: selectedPlayer?.i === player.i ? 
          '#ef4444' : 
          showTotalPoints ? "var(--total-points-color)" : "var(--avg-points-color)"
      })),
  ...
  useEffect(() => {
    ...
    fetch('/AdvancedManager/detailed_players.json', { signal: controller.signal })
      .then(response => response.json())
      .then((data: { players: { [key: string]: PlayerData } }) => {
        const available = Object.values(data.players)
          .map(player => ({
            ...player,
            ppm: player.mv > 0 ? Number((player.ap / (player.mv / 1000000)).toFixed(2)) : 0
          }))
          .sort((a, b) => b.mv - a.mv);
      ...
      <div className="header-container">
        <div className="flex flex-wrap items-center justify-between w-full gap-4">
          <div className="flex flex-wrap items-center gap-4">
            <h2>Marktwert gegen {showTotalPoints ? 'Gesamtpunkte' : 'Durschnittspunkte'}</h2>
            <PlayerSearch 
        ...
        <ResponsiveContainer width="100%" height={600}>
          ...
          >
            ...
            <XAxis 
              ...
              domain={['auto', 'auto']}
              label={{ value: 'Marktwert in Millionen', position: 'bottom', offset: 5 }}
              stroke="#666"
            />
            <YAxis 
              type="number"
              ...
              domain={[0,]}
              label={{ 
                value: showTotalPoints ? 'Gesamtpunkte' : 'Durschnittspunkte', 
                angle: -90, 
                position: 'insideLeft', 
                offset: -5 
              }}
            ...
            <Scatter
              name="Players"
              data={scatterData}
              fill={showTotalPoints ? "var(--total-points-color)" : "var(--avg-points-color)"}
