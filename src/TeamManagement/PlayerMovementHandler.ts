...
export class PlayerMovementHandler {
    ...
    static async handlePlayerMovement(
        ...
        if (destination.droppableId === 'auto-formation') {
            ...
            const newActivePoints = bestEleven.reduce(
            ...
            console.log(
                `Verbesserung: ${(newActivePoints - previousActivePoints).toFixed(2)} Punkte`
            )
        ...
        if (destination.droppableId === 'bench') {
            ...
            newPlayers = this.handleMoveFromBench(
                movedPlayer,
                destination,
                newPlayers
            )
        } else {
            newPlayers = this.handleRepositioning(
                movedPlayer,
                destination,
                newPlayers
            )
     ...
     */
    private static handleMoveToBench(
        movedPlayer: Player,
        players: Player[]
    ): Player[] {
        ...
        return [
            ...players.filter((p) => p.dayStatus === 1),
            ...sortedBenchPlayers,
        ]
    }
    ...
    private static handleMoveFromBench(
        ...
        const positionPlayers = players.filter(
            (p) => p.position === movedPlayer.position && p.dayStatus === 1
        )
        ...
        const insertIndex = players.findIndex(
            (p) => p.position === movedPlayer.position && p.dayStatus === 1
        )
    ...
    private static handleRepositioning(
        ...
        if (destination.index >= positionPlayers.length) {
            ...
            const positionStartIndex = players.findIndex(
                (p) => p.position === movedPlayer.position && p.dayStatus === 1
            )
