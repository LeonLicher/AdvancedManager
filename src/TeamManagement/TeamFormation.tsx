...
const TeamFormation: React.FC<TeamFormationProps> = ({
    ...
    const positions: PositionConfig[] = [
        {
            name: 'forwards',
            filter: (p: Player) => p.position === 4 && p.dayStatus === 1,
        },
        {
            name: 'midfielders',
            filter: (p: Player) => p.position === 3 && p.dayStatus === 1,
        },
        {
            name: 'defenders',
            filter: (p: Player) => p.position === 2 && p.dayStatus === 1,
        },
        {
            name: 'goalkeepers',
            filter: (p: Player) => p.position === 1 && p.dayStatus === 1,
        },
    ]
