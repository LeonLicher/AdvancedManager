...
const TeamManagement: React.FC<TeamManagementProps> = ({
    ...
    const handleGenerateBestEleven = async () => {
        ...
        try {
            ...
            const bestEleven = await generateBestEleven(
                preparedPlayers,
                httpClient,
                selectedFilter
            )
            ...
            if (bestEleven && bestEleven.length > 0) {
                ...
                const updatedPlayers = players.map((player) => ({
                    ...player,
                    dayStatus: bestEleven.some((p) => p.id === player.id)
                        ? 1
                        : 0,
                }))
                ...
                console.log(
                    `Generated best eleven with formation: ${formation}`
                )
            } else {
                setOptimizationError(
                    'Could not generate best eleven. Try again later.'
                )
            }
        } catch (error) {
            ...
            setOptimizationError(
                error instanceof Error
                    ? error.message
                    : 'Failed to generate best eleven'
            )
        } finally {
    ...
    return (
        <DragDropContext onDragStart={onDragStart} onDragEnd={onDragEnd}>
            <div className="team-management">
                ...
                />
                <Bench players={players} highlightedZones={highlightedZones} />
            </div>
        </DragDropContext>
    )
}
