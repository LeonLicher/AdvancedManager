...
export const useTeamManagement = (leagueId: string, userId: string) => {
                ...
                setPlayers(
                    data.it.map((player) => {
                        ...
                        for (const [shortName, longName] of Object.entries(
                            ...
                            mappedPlayer[longName as keyof Player] = player[
                                shortName as keyof typeof player
                            ] as any
                        }
                        // player.
    ...
    const updateFormation = useCallback(
        async (newPlayers: Player[]) => {
            ...
            // Parse the formation
            const [defCount, midCount, fwdCount] = formationType
                .split('-')
                .map(Number)
    ...
    const highlightValidDropZones = useCallback(
        (player: Player) => {
            ...
            const samePositionPlayers = players
                .filter(
    ...
    const onDragEnd = useCallback(
        async (result: DropResult) => {
            ...
            const newPlayers = await PlayerMovementHandler.handlePlayerMovement(
                result,
                players
            )
            if (newPlayers) {
