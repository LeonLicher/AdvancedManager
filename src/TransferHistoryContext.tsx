...
const TransferHistoryContext = createContext<TransferHistoryContextType | null>(
    null
)
...
export const TransferHistoryProvider: React.FC<{
    ...
    const getTransferHistory = async (
        ...
        if (
            cache[cacheKey] &&
            now - cache[cacheKey].timestamp < CACHE_EXPIRATION
        ) {
        ...
        try {
            const response = await httpClient.get<{
                it: TransferHistoryItem[]
            }>(
                `/v4/leagues/${leagueId}/players/${playerId}/transferHistory?start=1`
            )
            const data = response
    ...
    return (
        <TransferHistoryContext.Provider
            value={{ getTransferHistory, clearCache }}
        >
            {children}
        </TransferHistoryContext.Provider>
    )
...
export const useTransferHistory = () => {
    ...
    if (!context) {
        throw new Error(
            'useTransferHistory must be used within a TransferHistoryProvider'
        )
    }
    return context
}
