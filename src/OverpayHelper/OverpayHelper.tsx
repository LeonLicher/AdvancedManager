...
const OverpayHelper: React.FC<OverpayHelperProps> = ({ leagueId }) => {
    ...
    const analyzePlayer = async (player: Player) => {
        try {
            ...
            const alternativesResponse = await httpClient.post<{
                players: {
                    id: string
            ...
            const [history, mvResponse] = await Promise.all([
                ...
                httpClient.get<MarketValueResponse>(
                    `/v4/competitions/1/players/${player.i}/marketValue/365?leagueId=${leagueId}`
                ),
            ])

            // Analyze main player's purchases
            const mainPlayerAnalysis = await analyzeTransferHistory(
                history,
                mvResponse
            )
            ...
            const alternativeAnalyses = await Promise.all(
                alternativesResponse.players[0].analysis.alternatives.map(
                    async (alt) => {
                        const [altHistory, altMvResponse] = await Promise.all([
                            ...
                            httpClient.get<MarketValueResponse>(
                                `/v4/competitions/1/players/${alt.id}/marketValue/365?leagueId=${leagueId}`
                            ),
                        ])

                        const analysis = await analyzeTransferHistory(
                            altHistory,
                            altMvResponse
                        )
            ...
            const averageOverpayRatio =
                alternativeAnalyses.length > 0
                    ? alternativeAnalyses.reduce(
                          (sum, alt) => sum + alt.averageOverpayRatio,
                          0
                      ) / alternativeAnalyses.length
            ...
            const confidence = Math.min(
                alternativeAnalyses.reduce(
                    (sum, alt) => sum + alt.recentSales.length,
                    0
                ) * 1, // 1.6% per purchase
                100
            )
    ...
    const analyzeTransferHistory = async (
        history: any[],
        mvResponse: MarketValueResponse
    ) => {
        ...
        const purchaseAnalysis = history
            ...
            .map((sale: TransferHistoryItem) => {
                ...
                const dayNumber = Math.floor(
                    saleDate.getTime() / (1000 * 60 * 60 * 24)
                )

                // Find closest market value
                let mvAtTime = mvResponse.it[0]?.mv || 0
                const dayNumbers = Array.from(mvByDate.keys())
                    .map(Number)
                    .sort((a, b) => a - b)
        ...
        const averageOverpayRatio =
            purchaseAnalysis.length > 0
                ? purchaseAnalysis.reduce((sum, p) => sum + p.ratio, 0) /
                  purchaseAnalysis.length
                : 1.2
            ...
            {selectedPlayer && analysis ? (
                <div className="analysis-results">
                    <div className="player-info">
                        <h3>
                            {selectedPlayer.n} {selectedPlayer.ln}
                        </h3>
                        <div className="stats">
                            <div>
                                Marktwert:{' '}
                                {(selectedPlayer.mv / 1000000).toFixed(1)}M
                            </div>
                            <div>
                                Durchschnitt:{' '}
                                {selectedPlayer.ap
                                    ? selectedPlayer.ap.toFixed(1)
                                    : 'N/A'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bid-suggestion">
                        <div className="suggested-bid">
                            ...
                            <div className="amount">
                                {(analysis.suggestedBid / 1000000).toFixed(1)}M
                            </div>
                            ...
                            <div className="average-ratio">
                                Ø Overpay:{' '}
                                {(
                                    analysis.averageOverpayRatio * 100 -
                                    100
                                ).toFixed(0)}
                                %
                            </div>
                        ...
                        <div className="analysis-details">
                            <div className="alternatives">
                                    ...
                                    <TableBody>
                                        {(() => {
                                            const sortedAlternatives = [
                                                ...
                                                (a, b) =>
                                                    a.averageOverpayRatio -
                                                    b.averageOverpayRatio
                                            )
                                            return sortedAlternatives.map(
                                                (alt, index) => (
                                                    <TableRow key={index}>
                                                        <TableCell>
                                                            {alt.name}
                                ...
                                <div className="alternatives-summary">
                                    <span>
                                        Durchschnittlicher Overpay ähnlicher
                                        Spieler:{' '}
                                    </span>
                                    <span className="font-medium">
                                        {(
                                            (analysis.alternatives.reduce(
                                                (sum, alt) =>
                                                    sum +
                                                    alt.averageOverpayRatio,
                                                0
                ...
                <div className="analysis-results">
                    <div className="player-info">
                        <h3>
                            <Skeleton className="h-8 w-[200px] max-w-full" />
                        </h3>
                        <div className="stats">
                            <div>
                                Marktwert:{' '}
