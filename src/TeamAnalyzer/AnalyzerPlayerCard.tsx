  ...
  return (
    <div className="analyzer-player-card-wrapper">
          ...
          <div className="alternative-card">
            <div className="alternative-details">
              ...
              <div className="stats">
                ...
                <span>MV: {(player.mv / 1000000).toFixed(1)}M</span>
                {player.currentOwner && (
                  <>
                    <span className="owner">{player.currentOwner.name}</span>
                    <span>{(player.currentOwner.price / 1000000).toFixed(1)}M</span>
                  </>
                )}
              </div>
            </div>
            ...
            >
              ...
              <div className="alternative-details">
                ...
                <div className="stats">
                  <span>TP: {alt.tp}</span>
                  <span>AP: {alt.ap}</span>
                  <span>MV: {(alt.mv / 1000000).toFixed(1)}M</span>
                  {alt.currentOwner && (
                    <>
                      <span className="owner">{alt.currentOwner.name}</span>
                      <span>{(alt.currentOwner.price / 1000000).toFixed(1)}M</span>
                    </>
                  )}
                </div>
              </div>
      ...
      >
        ...
        />
        <div className="score">
          {score !== undefined ? score.toFixed(1) : <Skeleton className="h-6 w-[40px] my-2" />}
        </div>
        <div className="name">{player.n}</div>
