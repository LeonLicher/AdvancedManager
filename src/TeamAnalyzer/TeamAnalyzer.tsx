...
const calculateAdjustedScore = (
    ...
    const alternativeScores = alternatives.map((alt) => {
        ...
        const priceDiff = (() => {
            ...
            if (player.currentOwner && alt.currentOwner) {
                return (
                    (alt.currentOwner.price - player.currentOwner.price) /
    ...
    const averageAlternativeScore =
        alternativeScores.reduce((sum, score) => sum + score, 0) /
        alternativeScores.length
...
const TeamAnalyzer: React.FC<TeamAnalyzerProps> = ({ leagueId, userId }) => {
    ...
    const teamScore = useMemo(() => {
        ...
        return (
            ...
            players
                .filter((p) => p.mdst === 1)
                .reduce((sum, p) => sum + (p.analysisScore || 0), 0) /
                11
        ).toFixed(1)
    }, [players])
    ...
    useEffect(() => {
        const fetchSquad = async () => {
            ...
            setError(null)
            try {
                const data = await httpClient.get<{ it: Player[] }>(
    ...
    const analyzeTeam = async () => {
        ...
        try {
            const response = await httpClient.post<{
                players: { id: string; analysis: AnalysisResponse }[]
            }>('/api/analysis/team', {
            ...
            const analyzedPlayers = await Promise.all(
                players.map(async (player) => {
                    const analysisResult = response.players?.find(
                        (p) => p?.id === player.i
                    )
                    ...
                    try {
                        const playerHistory =
                            await transferHistory.getTransferHistory(
                                leagueId,
                                player.i
                            )
                        const lastTransfer =
                        ...
                        console.error(
                            'Error fetching player transfer history:',
                            error
                        )
                    }
                    ...
                    const alternativesWithOwners = await Promise.all(
                        analysisResult.analysis.alternatives.map(
                            async (alt) => {
                                try {
                                    ...
                                    console.error(
                                        'Error fetching alternative transfer history:',
                                        error
                                    )
                                    return alt
                                }
    ...
    const renderFormation = () => {
        const positions = [
            {
                name: 'forwards',
                filter: (p: Player) => p.pos === 4 && p.mdst === 1,
            },
            {
                name: 'midfielders',
                filter: (p: Player) => p.pos === 3 && p.mdst === 1,
            },
            {
                name: 'defenders',
                filter: (p: Player) => p.pos === 2 && p.mdst === 1,
            },
            {
                name: 'goalkeepers',
                filter: (p: Player) => p.pos === 1 && p.mdst === 1,
            },
        ]
                ...
                <div className="team-stats">
                    <div className="stat">
                        ...
                        <span className="value">
                            {players.some(
                                (p) => p.analysisScore !== undefined
                            ) ? (
                                teamScore
                            ) : (
                                <Skeleton className="h-8 w-[60px] my-3" />
                            )}
                        </span>
                    </div>
                </div>
